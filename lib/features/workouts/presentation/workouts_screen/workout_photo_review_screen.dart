// lib/presentation/workouts_screen/workout_photo_review_screen.dart
//
// Shows the exercises extracted from the photo, lets the user confirm/correct
// fuzzy matches and remove not-found ones, then pushes CreateWorkoutScreen
// pre-populated with the confirmed exercises.
...
class WorkoutPhotoReviewScreen extends StatefulWidget {
  final List<MatchedExercise> matches;

  const WorkoutPhotoReviewScreen({Key? key, required this.matches})
      : super(key: key);

  @override
  State<WorkoutPhotoReviewScreen> createState() =>
      _WorkoutPhotoReviewScreenState();
}

class _WorkoutPhotoReviewScreenState
    extends State<WorkoutPhotoReviewScreen> {
  ...
  void _proceed() {
    final confirmed = _items.where((i) => i.selected && i.match.match != null);
    ...
    // Monta um template pré-preenchido (id null = modo criação)
    final exerciseRows = confirmed.toList().asMap().entries.map((entry) {
      final ex    = entry.value.match.match!;
      final info  = entry.value.match.extracted;
      return TemplateExercise(
        orderIndex: entry.key + 1,
        sets: info.sets,
        reps: info.reps,
        restSeconds: info.rest,
        exerciseDbId: ex.exerciseId,
        exercise: Exercise(name: ex.name, exerciseDbId: ex.exerciseId),
      );
    }).toList();
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 10.w), // balance back button
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Padding(
            ...
            child: Wrap(
              spacing: 2.w,
              children: [
                _paramChip('${info.sets} séries',  Icons.repeat_rounded),
                _paramChip('${info.reps} reps',    Icons.fitness_center_rounded),
                _paramChip('${info.rest}s descanso', Icons.timer_outlined),
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          onPressed: selectedCount > 0 ? _proceed : null,
          ...
          child: Text(
            selectedCount > 0
                ? 'Montar treino com $selectedCount exercício${selectedCount != 1 ? "s" : ""}'
