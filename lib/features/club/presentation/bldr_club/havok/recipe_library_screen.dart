...
import 'package:bldr_fitness/features/club/presentation/bldr_club/havok/havok_hub.dart'; // Para as cores
import 'package:bldr_fitness/features/club/presentation/bldr_club/havok/recipe_results_screen.dart'; // Para a tela de detalhes
...
class _RecipeLibraryScreenState extends State<RecipeLibraryScreen> {
  ...
  Future<void> _fetchSavedRecipes() async {
    try {
      final result = await getIt<GetHavokRecipes>()();
      final rows = result.valueOrNull;
      if (rows == null) throw result.failureOrNull?.message ?? 'Erro';

      final recipes = rows.map(SavedRecipe.fromMap).toList();
