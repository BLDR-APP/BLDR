...
class _ComunidadeScreenState extends State<ComunidadeScreen>
  ...
  List<Map<String, dynamic>> _diversifyFeed(List<Map<String, dynamic>> items) {
    ...
    for (int i = 1; i < sorted.length; i++) {
      ...
      final sameType =
          _activityType(cur['reason_raw']) == _activityType(prev['reason_raw']);
      ...
      if (sameUser && sameType) {
        ...
        for (int j = i + 1; j < limit; j++) {
          ...
          final cSameType = _activityType(candidate['reason_raw']) ==
              _activityType(prev['reason_raw']);
  ...
  String _formatAction(String? raw, int xp) {
    ...
    if (r == 'workout_completed' || r == 'treino_completo')
      return 'concluiu um treino de musculação';
    ...
    if (r == 'nutrition_logged' || r == 'meal_logged')
      return 'registrou uma refeição';
    ...
    if (r.contains('treino') || r.contains('workout'))
      return 'concluiu um treino';
    if (r.contains('run') || r.contains('corrida'))
      return 'completou uma corrida';
    if (r.contains('hidrata') || r.contains('hydration'))
      return 'registrou hidratação';
    if (r.contains('pr') || r.contains('record'))
      return 'bateu um recorde pessoal';
  ...
  _ActivityType _activityType(String? raw) {
    ...
    if (r.contains('foto') || r.contains('progress'))
      return _ActivityType.photo;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: () =>
                Navigator.pushNamed(context, '/bldr-club/notificacoes'),
            child: Stack(
              children: [
          ...
          HavokEntryIcon(
            onTap: () =>
                showHavokSheet(context, originScreen: 'club_comunidade'),
          ),
        ],
      ),
    );
          ...
          if (_feedLoading)
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Center(
                    child: Text(
                        AppLocalizations.of(context).club_no_recent_activity,
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Text(AppLocalizations.of(context).club_ranking_week,
                  style: BldrText.label.copyWith(color: gold)),
              const Spacer(),
              GestureDetector(
                ...
                child: Text(AppLocalizations.of(context).club_see_all,
                    style:
                        BldrText.metaSm.copyWith(color: gold.withOpacity(0.7))),
              ),
            ],
          ),
                ...
                children: [
                  Text('#${myEntry.position}',
                      style: BldrText.cardTitle
                          .copyWith(color: gold, fontSize: 16)),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Text(
                      myEntry.displayName.split(' ')[0],
                      style:
                          BldrText.body.copyWith(fontWeight: FontWeight.w600),
                      overflow: TextOverflow.ellipsis,
                    ),
                  ),
                  Text('${myEntry.xp} xp',
                      style: BldrText.meta.copyWith(color: gold)),
                  if (nextEntry != null &&
                      nextEntry.userId != myEntry.userId) ...[
                    const SizedBox(width: 8),
                    Container(
          ...
          child: Column(
            children: [
              Icon(Icons.emoji_events_outlined,
                  color: gold.withOpacity(0.4), size: 36),
              const SizedBox(height: 12),
    ...
    return Padding(
      ...
      child: Column(
        children: _pastChallenges.map((c) {
          ...
          final endDate =
              DateTime.tryParse(c['ends_at'] as String? ?? '')?.toLocal();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text('Clique para acessar a comunidade exclusiva',
                    style: BldrText.meta),
              ],
            ),
          ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if ((challenge['description'] as String? ?? '')
                      .isNotEmpty) ...[
...
class _CreateChallengeModal extends StatefulWidget {
  const _CreateChallengeModal(
      {required this.myUserId, required this.onCreated});
...
class _CreateChallengeModalState extends State<_CreateChallengeModal> {
  ...
  static const _typeOptions = [
    {
      ...
      'label': 'XP Total',
      'icon': Icons.bolt,
      'desc': 'Comunidade acumula XP juntos'
    },
    {
      'value': 'workouts',
      'label': 'Treinos',
      'icon': Icons.fitness_center,
      'desc': 'Comunidade completa treinos'
    },
    {
      'value': 'streak',
      'label': 'Streak',
      'icon': Icons.local_fire_department,
      'desc': 'Todos mantêm sequência de dias'
    },
  ];
  ...
  String _buildTitle() {
    ...
    switch (_selectedType) {
      case 'workouts':
        return '💪 Desafio de $_selectedTarget Treinos';
      case 'streak':
        return '🔥 Streak de $_selectedDays Dias';
      default:
        return '⚡ Desafio de $_selectedTarget XP';
    }
  }
    ...
    try {
      ...
      final result = await getIt<CreateCollectiveChallenge>()(
        NewCollectiveChallenge(
          ...
          allowedSources:
              _selectedSources.isEmpty ? null : _selectedSources.toList(),
        ),
      );
      final failure = result.failureOrNull;
      if (failure != null) throw Exception(failure.message);
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            selected ? gold.withOpacity(0.15) : Colors.white10,
                        borderRadius: BorderRadius.circular(10),
                      ),
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(
                    color: isOn ? gold : Colors.white10, width: isOn ? 1.5 : 1),
              ),
              child: Row(
                children: [
                  Container(
          ...
          if (_useCustomTarget && _selectedType == 'xp_total') ...[
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Ex: 100000',
                hintStyle: TextStyle(color: Colors.grey[600], fontSize: 14),
                prefixIcon: const Icon(Icons.bolt, color: gold, size: 18),
                suffixText: 'XP',
                suffixStyle:
  ...
  Widget _buildStep4() {
    ...
    final typeLabel = (_typeOptions
        .firstWhere((t) => t['value'] == _selectedType)['label']) as String;
    final typeIcon = (_typeOptions
        .firstWhere((t) => t['value'] == _selectedType)['icon']) as IconData;
    final badge = _badgeController.text.trim();
  ...
  Widget _sectionLabel(String text) => Text(text,
      style:
          TextStyle(color: Colors.white54, fontSize: 10, letterSpacing: 1.5));
...
class _Blob extends StatelessWidget {
  const _Blob(
      {this.top, this.bottom, required this.opacity, required this.factor});
  final double? top;
  final double? bottom;
