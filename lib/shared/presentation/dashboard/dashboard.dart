...
class _DashboardState extends State<Dashboard> with TickerProviderStateMixin {
  ...
  Future<void> _checkReviewCriteria() async {
    ...
    try {
      ...
      final workoutsResult = await getIt<uc.GetWorkoutHistory>()(
        completedOnly: true,
        limit: 30,
      );
  ...
  void _startWorkout() async {
    try {
      ...
      if (workouts.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context).dashboard_no_workouts_available),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                    child: _buildQuickLogOption(AppLocalizations.of(context).dashboard_quick_log_meal,
                        'restaurant', AppTheme.successGreen, _logMeal)),
                SizedBox(width: 3.w),
                Expanded(
                    child: _buildQuickLogOption(AppLocalizations.of(context).dashboard_quick_log_workout,
                        'fitness_center', AppTheme.accentGold, _startWorkout)),
              ],
            ),
            SizedBox(height: 2.h),
            Row(
              children: [
                Expanded(
                  child: _buildQuickLogOption(
                      AppLocalizations.of(context).dashboard_quick_log_weight, 'monitor_weight', AppTheme.warningAmber,
                      () async {
                ...
                Expanded(
                  child: _buildQuickLogOption(
                      AppLocalizations.of(context).dashboard_quick_log_water, 'local_drink', Colors.blue, () async {
                    Navigator.pop(context);
                    try {
                      Navigator.pushNamed(context, AppRoutes.nutritionScreen);
                    } catch (e) {
