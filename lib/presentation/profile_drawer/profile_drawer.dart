...
import 'package:flutter/foundation.dart'; // <<< ESSENCIAL PARA kDebugMode E Platform
import 'dart:io'; // <<< ESSENCIAL PARA Platform.isIOS
...
class _ProfileDrawerState extends State<ProfileDrawer> {
  ...
  // ===============================================
  // ===== LÓGICA DE RETRY E TOKEN APNS (NOVO) =====
  // ===============================================
  ...
  Future<void> _waitForAPNSToken() async {
    ...
    for (int i = 0; i < retries; i++) {
      try {
        ...
        if (apnsToken != null) {
          if (kDebugMode) print('APNS token obtido após $i retries.');
          return; // Sucesso, sai da função
        }
      } catch (e) {
        // Ignora o erro 'apns-token-not-set', pois estamos esperando que ele desapareça
        if (kDebugMode) print('APNS Token indisponível, retrying... ($i/5)');
      }
      await Future.delayed(const Duration(seconds: 1)); // Aguarda 1 segundo
    }
    if (kDebugMode) print('APNS Token não obtido após retries.');
  }
    ...
    try {
      ...
      if (newValue == true) {
        ...
        if (settings.authorizationStatus == AuthorizationStatus.authorized) {
          ...
          // 2. PASSO CRÍTICO: FORÇAR ESPERA PELO TOKEN APNS NO IOS
          if (Platform.isIOS) {
            await _waitForAPNSToken();
          }

          // 3. Obter o token FCM (mais seguro agora)
          String? fcmToken = await fcm.getToken();

          if (fcmToken != null) {
            updates['fcm_token'] = fcmToken;
            if (kDebugMode) print('Token FCM obtido e pronto para salvar: $fcmToken');
          } else {
            // Se falhar mesmo após o retry, é um erro sério de configuração
        ...
        if (kDebugMode) print('Token FCM deletado.');
      }
  ...
  // --- As funções auxiliares foram mantidas dentro da classe ---
                      ...
                      items: [
                        ...
                        ProfileSectionItem(
                          ...
                          onTap: () {
                            ...
                            // Certifique-se de que 'AppRoutes.onboardingFlow' é a rota correta
